    ...
    >
      <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-3 lg:grid-rows[auto,auto]">
        <FadeIn className="translate-y-16 opacity-0 lg:col-span-2 lg:row-span-2" vars={{duration:1}} start="top 70%">
          <PrismicNextImage field={slice.primary.image} className="w-full h-auto object-cover" />
        </FadeIn>
        ...
        <FadeIn className="animate-in opacity-0 relative translate-y-16 self-end bg-white/10 will-change-transform" vars={{duration:1, delay:1}}>
          <PrismicNextImage field={fragrance?.data.bottle_image} className="mx-auto -mt-10 w-full -rotate-12 md:-mt-20" />
          <div className="flex justify-between p-10 pt-4">
            <div className="space-y-1">
              <h3 className="font-display text-4xl "><PrismicText field={fragrance?.data.title} fallback="Fragrance"/></h3>
              <p className="mt-2 text-gray-400">Eau de Parfum</p>
            <ButtonLink document={fragrance} variant="Secondary" className="mt-6">Shop Now</ButtonLink>
            </div>
